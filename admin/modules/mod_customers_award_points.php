...
if($objLogin->IsLoggedInAs('owner','mainadmin') && Modules::IsModuleInstalled('customers') && Modules::IsModuleInstalled('loyalty')){
	
	$action 		= MicroGrid::GetParameter('action');
	$rid    		= MicroGrid::GetParameter('rid');
	$customer_id	= (int)MicroGrid::GetParameter('cid', false);
	$mode   		= 'view';
	$msg 			= '';
	
	$objCustomers = new Customers();
	$customer_info = $objCustomers->GetCustomerInfo($customer_id);
	
	if(!empty($customer_id) && count($customer_info) > 0){	
		$objCustomerAwardPoints = new CustomerAwardPoints($customer_id);
		
		if($action=='add'){		
			$mode = 'add';
		}elseif($action=='create'){
			if($objCustomerAwardPoints->addRecord()){
				$msg = draw_success_message(_ADDING_OPERATION_COMPLETED, false);
				$mode = 'view';
			}else{
				$msg = draw_important_message($objCustomerAwardPoints->error, false);
				$mode = 'add';
			}
		}elseif($action=='edit'){
			$mode = 'edit';
		}elseif($action=='update'){
			if($objCustomerAwardPoints->UpdateRecord($rid)){
			...
			if($objCustomerAwardPoints->DeleteRecord($rid)){
			...
			if($objCustomerAwardPoints->RemoveRecord($rid, $comments)){
		...
		$customer_name = isset($customer_info['first_name']) ? $customer_info['first_name'].' '.$customer_info['last_name'] : '';
		...
		draw_title_bar(
			...
			prepare_permanent_link('index.php?admin=mod_customers_management', _BUTTON_BACK)
		);
		...
		draw_content_start();	
		if($mode == 'view'){
			$objCustomerAwardPoints->DrawViewMode();
			...
			$objCustomerAwardPoints->DrawAddMode();		
		}elseif($mode == 'edit'){		
			$objCustomerAwardPoints->DrawEditMode($rid);		
		}elseif($mode == 'details'){		
			$objCustomerAwardPoints->DrawDetailsMode($rid);		
		}
