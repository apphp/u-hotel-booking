<?php
/**
* @project uHotelBooking
* @copyright (c) 2019 ApPHP
* @author ApPHP <info@apphp.com>
* @site https://www.hotel-booking-script.com
* @license http://hotel-booking-script.com/license.php
*/

// *** Make sure the file isn't accessed directly
defined('APPHP_EXEC') or die('Restricted Access');
//--------------------------------------------------------------------------

if($objLogin->IsLoggedInAs('owner','mainadmin') && Modules::IsModuleInstalled('rooms')){	

	$action = MicroGrid::GetParameter('action');
	$rid    = MicroGrid::GetParameter('rid');
	$mode   = 'view';
	$msg    = '';
	
	$objRoomsSettings = new ModulesSettings('rooms');
	
	if($action=='add'){		
		$mode = 'add';
	}elseif($action=='create'){
		if($objRoomsSettings->AddRecord()){
			$msg = draw_success_message(_ADDING_OPERATION_COMPLETED, false);
			$mode = 'view';
		}else{
			$msg = draw_important_message($objRoomsSettings->error, false);
			$mode = 'add';
		}
	}elseif($action=='edit'){
		$mode = 'edit';
	}elseif($action=='update'){
		if($objRoomsSettings->UpdateRecord($rid)){
			$msg = draw_success_message(_UPDATING_OPERATION_COMPLETED, false);
			$mode = 'view';
		}else{
			$msg = draw_important_message($objRoomsSettings->error, false);
			$mode = 'edit';
		}		
	}elseif($action=='delete'){
		if($objRoomsSettings->DeleteRecord($rid)){
			$msg = draw_success_message(_DELETING_OPERATION_COMPLETED, false);
		}else{
			$msg = draw_important_message($objRoomsSettings->error, false);
		}
		$mode = 'view';
	}elseif($action=='details'){		
		$mode = 'details';		
	}elseif($action=='cancel_add'){		
		$mode = 'view';		
	}elseif($action=='cancel_edit'){				
		$mode = 'view';
	}
	
	// Start main content
    if(FLATS_INSTEAD_OF_HOTELS){
		draw_title_bar(prepare_breadcrumbs(array(
			_FLATS_MANAGEMENT	=> 'index.php?admin=hotels_info',
			_FLATS				=> 'index.php?admin=mod_rooms_management',
			_ROOMS_SETTINGS		=> (empty($action) ? '' : 'index.php?admin=mod_rooms_settings'),
			ucfirst($action)	=> ''
		)));
    }else{
		draw_title_bar(prepare_breadcrumbs(array(
			_HOTELS_MANAGEMENT	=> 'index.php?admin=hotels_info',
			_ROOMS				=> 'index.php?admin=mod_rooms_management',
			_ROOMS_SETTINGS		=> (empty($action) ? '' : 'index.php?admin=mod_rooms_settings'),
			ucfirst($action)	=> ''
		)));
    }
    echo '<br />';
	
	//if($objSession->IsMessage('notice')) echo $objSession->GetMessage('notice');
	echo $msg;

	draw_content_start();
	if($mode == 'view'){		
		$objRoomsSettings->DrawViewMode();	
	}elseif($mode == 'add'){		
		$objRoomsSettings->DrawAddMode();		
	}elseif($mode == 'edit'){		
		$objRoomsSettings->DrawEditMode($rid);		
	}elseif($mode == 'details'){ 
		$objRoomsSettings->DrawDetailsMode($rid);		
	}
	draw_content_end();

}else{
	draw_title_bar(_ADMIN);
	draw_important_message(_NOT_AUTHORIZED);
}

